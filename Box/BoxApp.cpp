...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	// 이 프레임의 영역들이 모두 처리되길 기다린다. 이러한 대기는 비효율적인다.
	//이번에는 예제의 간단함을 위해 이 방법을 사용하지만, 이후의 예제들에서는 렌더링 코드를 적절히 조직화해서
	//프레임마다 대기할 필요가 없게 만든다.
	FlushCommandQueue();
}
...
void BoxApp::OnMouseMove(WPARAM btnState, int x, int y)
{
    ...
    {
		// 마우스 한 픽셀 이동을 4분의 1도에 대응한다.
        float dx = XMConvertToRadians(0.25f*static_cast<float>(x - mLastMousePos.x));
        float dy = XMConvertToRadians(0.25f*static_cast<float>(y - mLastMousePos.y));

        //  마우스 입력에 기초해 각도를 갱신한다. 이에 의해 카메라가 상자를 중심으로 공전하게 됨
        mTheta += dx;
        mPhi += dy;
...
void BoxApp::BuildRootSignature()
{
	...
	입력된다고 기대함. 루트 서명은 셰이더 프로그램이 기대하는 자원들을 정의함.
	ㅖ이더 프로그램은 본질적으로 하나의 함수이고 셰이더에 입력되는 자원들은
	함수의 매개변수들에 해당하므로, 루트서명은 곧 함수 서명을 정의하는 수단이라 할 수 있다.
	*/

	// 루트 매개변수는 서술자 테이블이거나 루트 서술자 또는 루트 상수이다.
	CD3DX12_ROOT_PARAMETER slotRootParameter[1];

	// CPV 하나를 담는 서술자 테이블을 생성
