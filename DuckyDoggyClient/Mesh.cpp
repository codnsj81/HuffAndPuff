...
CWaterMesh::CWaterMesh(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, int nWidth, int nLength)
{
	...
	{
		m_pxmf4Colors[i] = XMFLOAT4(0, 1, 1, 0.2f); 
	}
...
void CWaterMesh::ReleaseUploadBuffers()
{
	CMesh::ReleaseUploadBuffers();
...
void CWaterMesh::OnPreRender(ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
