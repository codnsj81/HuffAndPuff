...
void LitColumnsApp::UpdateCamera(const GameTimer& gt)
{
	...
	XMVECTOR pos = XMVectorSet(mEyePos.x + mCameraMoving.x, mEyePos.y, mEyePos.z + mCameraMoving.z, 1.0f);
	XMVECTOR target =XMVectorSet(mCameraMoving.x,0, mCameraMoving.z,1);
...
void LitColumnsApp::AnimateMaterials(const GameTimer& gt)
{
	...
	if (GetAsyncKeyState(VK_RIGHT))
		mCameraMoving.x += gt.DeltaTime() * 5.f;
	if (GetAsyncKeyState(VK_LEFT))
		mCameraMoving.x -= gt.DeltaTime() *5.f;
	if (GetAsyncKeyState(VK_UP))
		mCameraMoving.z += gt.DeltaTime() * 5.f;
	if (GetAsyncKeyState(VK_DOWN))
		mCameraMoving.z -= gt.DeltaTime() *5.f;

	auto& rd = mAllRitems.front();
	if (GetAsyncKeyState('A'))
		rd->position.x -= gt.DeltaTime() * 5.f;
	if (GetAsyncKeyState('D'))
		rd->position.x += gt.DeltaTime() * 5.f;
	if (GetAsyncKeyState('W'))
		rd->position.z += gt.DeltaTime() * 5.f;
	if (GetAsyncKeyState('S'))
		rd->position.z -= gt.DeltaTime() * 5.f;


	XMStoreFloat4x4(&rd->World, XMMatrixScaling(rd->Scale.x, rd->Scale.y, rd->Scale.z)
		* XMMatrixTranslation(rd->position.x, rd->position.y, rd->position.z));
	rd->NumFramesDirty++;
...
void LitColumnsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.Lights[1].Direction = { 1.f, -0.57735f,0.f };
	mMainPassCB.Lights[1].Strength = { 0.3f, 0.3f, 0.3f };
	/*mMainPassCB.Lights[2].Direction = { 0.0f, -0.707f, -0.707f };
	mMainPassCB.Lights[2].Strength = { 0.3f, 0.3f, 0.3f };
	mMainPassCB.Lights[2].SpotPower = 100;*/
...
void LitColumnsApp::BuildMaterials()
{
	...
	stone0->DiffuseAlbedo = XMFLOAT4(Colors::Yellow);
	...
	tile0->DiffuseAlbedo = XMFLOAT4(Colors::Chocolate);
	tile0->FresnelR0 = XMFLOAT3(0.02f, 0.02f, 0.02f);
	tile0->Roughness = 0.2f;
	
	mMaterials["yellow0"] = std::move(stone0);
...
void LitColumnsApp::BuildRenderItems()
{
	...
	boxRitem->position.y = 1;
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(boxRitem->Scale.x, boxRitem->Scale.y, boxRitem->Scale.z) 
									* XMMatrixTranslation(boxRitem->position.x, boxRitem->position.y, boxRitem->position.z));

	XMStoreFloat4x4(&boxRitem->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));
	boxRitem->ObjCBIndex = 0;
	boxRitem->Mat = mMaterials["yellow0"].get();
