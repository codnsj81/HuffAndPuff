...
int main(int argc, char *argv[])
{
	...
	// 이벤트 객체 생성
	hReadEvent = CreateEvent(NULL, FALSE, TRUE, NULL); // WorkerThread 스레드가 client_sock 변수 값을 읽었음을 메인 스레드에 알리는 용도
	if (hReadEvent == NULL) return 1;
	hWriteEvent = CreateEvent(NULL, FALSE, FALSE, NULL); // 메인 스레드가 client_sock 변수 값을 변경했음을 alertable wait 상태인 WorkerThread 스레드에 알리는 용도
