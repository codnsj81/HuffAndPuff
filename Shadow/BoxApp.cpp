...
private:
	...
	XMFLOAT3 mBoxTranslation = { 0.0f, 0.5f, 0.0f };
...
void BoxApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	if (GetAsyncKeyState('A') & 0x8000)
		mBoxTranslation.x -= 1.0f*dt;

	if (GetAsyncKeyState('D') & 0x8000)
		mBoxTranslation.x += 1.0f*dt;

	if (GetAsyncKeyState('W') & 0x8000)
		mBoxTranslation.z += 1.0f*dt;

	if (GetAsyncKeyState('S') & 0x8000)
		mBoxTranslation.z -= 1.0f*dt;

	// Don't let user move below ground plane.
	mBoxTranslation.y = MathHelper::Max(mBoxTranslation.y, 0.0f);
	...
	XMStoreFloat4x4(&mShadowedBoxRitem->World, BoxWorld * S * shadowOffsetY);

	mBoxRitem->NumFramesDirty = gNumFrameResources;
	mShadowedBoxRitem->NumFramesDirty = gNumFrameResources;
}
...
void BoxApp::BuildMaterials()
{
	...
	Floor->DiffuseAlbedo = XMFLOAT4(Colors::Green);
	...
	shadowMat->DiffuseSrvHeapIndex = 1;
...
void BoxApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&floorRitem->World, XMMatrixTranslation(0, 0, 5));
	...
	*shadowedBoxRitem = *BoxRitem;
	shadowedBoxRitem->ObjCBIndex = 2;
	shadowedBoxRitem->Mat = mMaterials["shadowMat"].get();
	mShadowedBoxRitem = shadowedBoxRitem.get();
